...
def main():
    ...
    output_path = (fig_dir / "value_lf_kalman_filtered_150_160.png").resolve()
    ...
    local_times = []
    local_values_lf = []
    local_filtered_values_lf = []
    for time_value, raw_value, filtered_value in zip(
        relative_times, values_lf, filtered_values_lf
    ):
        if LOCAL_START_TIME <= time_value <= LOCAL_END_TIME:
            local_times.append(time_value)
            local_values_lf.append(raw_value)
            local_filtered_values_lf.append(filtered_value)
    ...
    plot_figure(
        ...
        local_filtered_values_lf,
        output_path,
        f"Value_LF with Kalman Filtering ({LOCAL_START_TIME:.0f}s to {LOCAL_END_TIME:.0f}s)",
        1.4,
        2.4,
    )
