...
def calc_contact_state(force_value, threshold):
    if not math.isfinite(force_value):
        return 0
    return 0 if force_value < threshold else 1
...
def build_labeled_rows(rows, config):
    ...
    for row in rows:
        ...
        for leg, settings in LEG_SETTINGS.items():
            ...
            labeled_row[f"{leg}_contact_states"] = calc_contact_state(
                force_value,
                settings["threshold"],
            )
...
def save_labeled_csv(output_path: Path, fieldnames, labeled_rows):
    ...
    output_fields = list(fieldnames) + contact_fields
...
def main():
    ...
    output_dir = save_dir / "Force"
    output_dir.mkdir(parents=True, exist_ok=True)
    output_csv_path = output_dir / "data_label.csv"
    ...
    print("Thresholds:")
    for leg, settings in LEG_SETTINGS.items():
        print(f"  {leg}: {settings['threshold']:g}")
