...
def load_lh_joint_and_torque(csv_path: Path):
    relative_times = []
    lh_joint_angles = []
    lh_joint_torques = []
    start_time = None
    ...
    with csv_path.open("r", encoding="utf-8", newline="") as csv_file:
        ...
        for row_number, row in enumerate(reader, start=2):
            try:
                ...
                tau1 = float(row["LH_HAA_effort"])
                tau2 = float(row["LH_HFE_effort"])
                tau3 = float(row["LH_KFE_effort"])
            ...
            lh_joint_torques.append((tau1, tau2, tau3))

    if not relative_times:
        raise ValueError(f"No valid LH joint data found in {csv_path}")

    return relative_times, lh_joint_angles, lh_joint_torques


def calc_lh_foot_force_z(joint_angles, joint_torques, config):
...
def save_lh_foot_force_z_csv(output_path: Path, relative_times, foot_force_z):
    with output_path.open("w", encoding="utf-8", newline="") as csv_file:
        writer = csv.writer(csv_file)
        writer.writerow(["time", "LH_foot_force_z"])
...
def main():
    ...
    output_csv_path = save_dir / "LH" / "data_lh_foot_z.csv"
    config_path = (script_dir / "../../config/LimxDynamic_WL_Config.yaml").resolve()
    fig_dir = save_dir / "LH" / "fig"
    fig_dir.mkdir(parents=True, exist_ok=True)
    output_path = fig_dir / "lh_foot_force_z_12_20.png"

    config = load_wl_config(config_path)
    relative_times, lh_joint_angles, lh_joint_torques = load_lh_joint_and_torque(csv_path)
    lh_foot_force_z = calc_lh_foot_force_z(lh_joint_angles, lh_joint_torques, config)
    save_lh_foot_force_z_csv(output_csv_path, relative_times, lh_foot_force_z)

    local_times = []
    local_force_z = []
    for time_value, force_z in zip(relative_times, lh_foot_force_z):
    ...
    plt.ylabel("LH foot_force_z")
    plt.title("LH foot_force_z (12s to 20s)")
