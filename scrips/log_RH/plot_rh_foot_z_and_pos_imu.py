...
def parse_args():
    parser = argparse.ArgumentParser(
        description=(
            "Read RH foot force and RH foot position CSV files, then plot a selected "
    ...
    parser.add_argument(
        ...
        type=Path,
        default=None,
        help="Optional path to the RH foot force CSV file",
    )
    parser.add_argument(
        "--pos-csv",
        type=Path,
        default=None,
        help="Optional path to the RH foot position IMU CSV file",
    )
    parser.add_argument(
        "--output",
        type=Path,
...
def main():
    ...
    pos_csv_path = resolve_input_path(
        save_dir / "RH",
        args.pos_csv,
        ["data_rh_foot_pos_imu_z.csv"],
    )

    if args.output is None:
        range_token = f"{format_range_token(args.start)}_{format_range_token(args.end)}"
        output_path = fig_dir / f"rh_foot_z_and_pos_imu_{range_token}.png"
    else:
    ...
    axis_force.set_title(
        f"RH foot force and RH foot pos imu z ({args.start}s to {args.end}s)"
    )
