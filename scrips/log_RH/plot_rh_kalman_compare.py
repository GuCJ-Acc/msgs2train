...
def parse_args():
    parser = argparse.ArgumentParser(
        description=(
            "Read raw RH data from data.csv and filtered RH data from "
            "data_rh_kalman.csv, then plot them for a selected time range."
        )
    )
    parser.add_argument(
        "--start",
    ...
    parser.add_argument(
        ...
        type=Path,
        default=None,
        help="Optional path to raw RH CSV file",
    )
    parser.add_argument(
        "--kalman-csv",
        type=Path,
        default=None,
        help="Optional path to filtered RH CSV file",
    )
    parser.add_argument(
        "--output",
        type=Path,
...
def load_raw_rh_data(csv_path: Path):
    relative_times = []
    rh_values = []
    start_time = None
...
def load_kalman_rh_data(csv_path: Path):
    relative_times = []
    rh_values = []
    ...
    if not relative_times:
        raise ValueError(f"No valid RH kalman data found in {csv_path}")
...
def main():
    ...
    if args.output is None:
        ...
        output_path = fig_dir / f"rh_kalman_compare_{range_token}.png"
    else:
        output_path = args.output.resolve()

    raw_times, raw_values = load_raw_rh_data(raw_csv_path)
    kalman_times, kalman_values = load_kalman_rh_data(kalman_csv_path)
    ...
    require_nonempty(raw_times, args.start, args.end, "raw RH data", raw_csv_path)
    require_nonempty(
        kalman_times,
        args.start,
        args.end,
    ...
    plt.ylabel("RH")
    plt.title(f"RH raw vs kalman ({args.start}s to {args.end}s)")
