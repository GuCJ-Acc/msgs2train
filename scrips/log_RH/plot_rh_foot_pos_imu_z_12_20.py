...
def load_rh_joint_angles(csv_path: Path):
    relative_times = []
    rh_joint_angles = []
    start_time = None
    ...
    return relative_times, rh_joint_angles


def calc_rh_foot_pos_imu_z(joint_angles, config):
...
def save_rh_foot_pos_imu_z_csv(output_path: Path, relative_times, foot_pos_imu_z):
    with output_path.open("w", encoding="utf-8", newline="") as csv_file:
        writer = csv.writer(csv_file)
        writer.writerow(["time", "RH_foot_pos_imu_z"])
...
def main():
    ...
    output_csv_path = save_dir / "RH" / "data_rh_foot_pos_imu_z.csv"
    config_path = (script_dir / "../../config/LimxDynamic_WL_Config.yaml").resolve()
    fig_dir = save_dir / "RH" / "fig"
    fig_dir.mkdir(parents=True, exist_ok=True)
    output_path = fig_dir / "rh_foot_pos_imu_z_12_20.png"

    config = load_wl_config(config_path)
    relative_times, rh_joint_angles = load_rh_joint_angles(csv_path)
    rh_foot_pos_imu_z = calc_rh_foot_pos_imu_z(rh_joint_angles, config)
    save_rh_foot_pos_imu_z_csv(output_csv_path, relative_times, rh_foot_pos_imu_z)

    local_times = []
    local_foot_pos_imu_z = []
    for time_value, foot_z in zip(relative_times, rh_foot_pos_imu_z):
    ...
    plt.ylabel("RH foot_pos_imu z (m)")
    plt.title("RH foot_pos_imu z (12s to 20s)")
