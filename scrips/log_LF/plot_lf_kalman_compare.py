...
def parse_args():
    parser = argparse.ArgumentParser(
        description=(
            "Read raw LF data from data.csv and filtered LF data from "
            "data_lf_kalman.csv, then plot them for a selected time range."
        )
    )
    parser.add_argument(
        "--start",
    ...
    parser.add_argument(
        ...
        type=Path,
        default=None,
        help="Optional path to raw LF CSV file",
    )
    parser.add_argument(
        "--kalman-csv",
        type=Path,
        default=None,
        help="Optional path to filtered LF CSV file",
    )
    parser.add_argument(
        "--output",
        type=Path,
...
def load_raw_lf_data(csv_path: Path):
    relative_times = []
    lf_values = []
    start_time = None
...
def load_kalman_lf_data(csv_path: Path):
    relative_times = []
    lf_values = []
    ...
    if not relative_times:
        raise ValueError(f"No valid LF kalman data found in {csv_path}")
...
def resolve_path(script_dir: Path, explicit_path: Optional[Path], default_name: str):
...
def main():
    ...
    kalman_csv_path = resolve_path(save_dir / "LF", args.kalman_csv, "data_lf_kalman.csv")

    if args.output is None:
        range_token = f"{format_range_token(args.start)}_{format_range_token(args.end)}"
        output_path = fig_dir / f"lf_kalman_compare_{range_token}.png"
    else:
        output_path = args.output.resolve()

    raw_times, raw_values = load_raw_lf_data(raw_csv_path)
    kalman_times, kalman_values = load_kalman_lf_data(kalman_csv_path)
    ...
    require_nonempty(raw_times, args.start, args.end, "raw LF data", raw_csv_path)
    require_nonempty(
        kalman_times,
        args.start,
        args.end,
    ...
    plt.ylabel("LF")
    plt.title(f"LF raw vs kalman ({args.start}s to {args.end}s)")
