...
def load_rf_joint_and_torque(csv_path: Path):
    relative_times = []
    rf_joint_angles = []
    rf_joint_torques = []
    start_time = None
    ...
    with csv_path.open("r", encoding="utf-8", newline="") as csv_file:
        ...
        for row_number, row in enumerate(reader, start=2):
            try:
                ...
                q1 = float(row["RF_HAA_position"])
                q2 = float(row["RF_HFE_position"])
                q3 = float(row["RF_KFE_position"])
                tau1 = float(row["RF_HAA_effort"])
                tau2 = float(row["RF_HFE_effort"])
                tau3 = float(row["RF_KFE_effort"])
            ...
            rf_joint_angles.append((q1, q2, q3))
            rf_joint_torques.append((tau1, tau2, tau3))

    if not relative_times:
        raise ValueError(f"No valid RF joint data found in {csv_path}")

    return relative_times, rf_joint_angles, rf_joint_torques


def calc_rf_foot_force_z(joint_angles, joint_torques, config):
    ...
    for (q1_raw, q2_raw, q3_raw), (tau1, tau2, tau3) in zip(
        ...
        # For RF in calculateFootState_Body():
...
def save_rf_foot_force_z_csv(output_path: Path, relative_times, foot_force_z):
    with output_path.open("w", encoding="utf-8", newline="") as csv_file:
        writer = csv.writer(csv_file)
        writer.writerow(["time", "RF_foot_force_z"])
...
def main():
    ...
    output_csv_path = save_dir / "RF" / "data_rf_foot_z.csv"
    config_path = (script_dir / "../../config/LimxDynamic_WL_Config.yaml").resolve()
    fig_dir = save_dir / "RF" / "fig"
    fig_dir.mkdir(parents=True, exist_ok=True)
    output_path = fig_dir / "rf_foot_force_z_12_20.png"

    config = load_wl_config(config_path)
    relative_times, rf_joint_angles, rf_joint_torques = load_rf_joint_and_torque(csv_path)
    rf_foot_force_z = calc_rf_foot_force_z(rf_joint_angles, rf_joint_torques, config)
    save_rf_foot_force_z_csv(output_csv_path, relative_times, rf_foot_force_z)

    local_times = []
    local_force_z = []
    for time_value, force_z in zip(relative_times, rf_foot_force_z):
    ...
    plt.ylabel("RF foot_force_z")
    plt.title("RF foot_force_z (12s to 20s)")
